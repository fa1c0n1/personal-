...
@Service
@ConditionalOnProperty(prefix = "stargate.cache.ofs", name = "type", havingValue = "redis-multi")
public class RedisMultiValueCacheService implements CacheService {
    ...
    public <O> O getCacheValue(final String cacheName, final String key) throws Exception {
        ...
        try {
            RSetMultimap<String, Object> cache = redisClient.getSetMultimap(cacheName);
            return (O) cache.getAll(key);
        } catch (Exception e) {
    ...
    public Mono<Object> getKVCacheValue(final String cacheName, final String key) {
        long startTimeNanos = System.nanoTime();
        try {
            RSetMultimapReactive<String, Object> cache = redisClient.reactive().getSetMultimap(cacheName);
            Mono<Object> value = cache.getAll(key).map(values -> values);
            return value;
        } catch (Exception e) {
            readErrorCounter.labels("redis", "getKVCacheValue").inc();
            return Mono.error(e);
    ...
    public Mono<Boolean> deleteKVCacheValue(final String cacheName, final String key) {
        ...
        try {
            RSetMultimapReactive<String, Object> cache = redisClient.reactive().getSetMultimap(cacheName);
            Mono<Boolean> result = cache.removeAll(key).map(value -> value != null);
            return result;
        } catch (Exception e) {
    ...
    public Mono<Collection<String>> getKVKeys(final String cacheName) {
        return Mono.create(sink -> {
            long startTimeNanos = System.nanoTime();
            try {
                RSetMultimap<String, Object> cache = redisClient.getSetMultimapCache(cacheName);
                sink.success(cache.readAllKeySet());
            } catch (Exception e) {
