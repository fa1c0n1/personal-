...
public class WorkerDeploymentPrecondition implements Condition<Deployment, CoreResource> {
    private final Logger logger = logger(MethodHandles.lookup().lookupClass());

    @Override
    public boolean isMet(final DependentResource<Deployment, CoreResource> dependentResource, final CoreResource primary, final Context<CoreResource> context) {
