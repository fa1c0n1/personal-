...
@Deprecated
public class CoordinatorStatefulSetPrecondition implements Condition<StatefulSet, CoreResource> {
    private final Logger logger = logger(MethodHandles.lookup().lookupClass());

    @Override
    public boolean isMet(final DependentResource<StatefulSet, CoreResource> dependentResource, final CoreResource primary, final Context<CoreResource> context) {
