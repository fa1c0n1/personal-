...
import static com.apple.aml.stargate.common.constants.CommonConstants.MetricStages.QUERY_EXECUTED;
import static com.apple.aml.stargate.pipeline.sdk.utils.PipelineUtils.histogram;

/**
 * Fixed issue for missing records from the Cassandra Table
 * Git Commit # ad72baceaef2c3c0b44d0b9538689bd1eecc2d18
 * Git Issue https://github.com/apache/beam/issues/24829
 *
 * @param <T>
 */

@SuppressWarnings({"rawtypes", // TODO(https://github.com/apache/beam/issues/20447)
...
class ReadFn<T> extends DoFn<Read<T>, T> {
        ...
        try {
            ...
            for (RingRange rr : ringRanges) {
                ...
                if (rr.isWrapping()) {
                    ...
                    ResultSet rs = session.execute(getLowestSplitQuery(read, partitionKey, rr.getEnd()));
                    /** Changes with respect to Support ACI Cassandra */
                    histogram("cassandra-reader", "cassandra", "unknown", QUERY_EXECUTED).observe((System.nanoTime() - startTime) / 1000000.0);
                    /** End of ACI Cassandra changes Block */
                    ...
                    histogram("cassandra-reader", "cassandra", "unknown", QUERY_EXECUTED).observe((System.nanoTime() - startTime) / 1000000.0);
                    /** End of ACI Cassandra changes Block */
                    rs = session.execute(getHighestSplitQuery(read, partitionKey, rr.getStart()));
                    /** Changes with respect to Support ACI Cassandra */
                    histogram("cassandra-reader", "cassandra", "unknown", QUERY_EXECUTED).observe((System.nanoTime() - startTime) / 1000000.0);
                    /** End of ACI Cassandra changes Block */
                    outputResults(rs, receiver, mapper);
    ...
    private static <T> void outputResults(ResultSet rs, OutputReceiver<T> outputReceiver, Mapper<T> mapper) {
    ...
    private static String getHighestSplitQuery(Read<?> spec, String partitionKey, BigInteger highest) {
        String highestClause = String.format("(token(%s) >= %d)", partitionKey, highest);
        String finalHighQuery = (spec.query() == null) ? buildInitialQuery(spec, true) + highestClause : spec.query() + " AND " + highestClause;
    ...
    private static String getLowestSplitQuery(Read<?> spec, String partitionKey, BigInteger lowest) {
        ...
        String finalLowQuery = (spec.query() == null) ? buildInitialQuery(spec, true) + lowestClause : spec.query() + " AND " + lowestClause;
    ...
    private static String generateRangeQuery(Read<?> spec, String partitionKey, Boolean hasRingRange) {
        final String rangeFilter = hasRingRange ? Joiner.on(" AND ").skipNulls().join(String.format("(token(%s) >= ?)", partitionKey), String.format("(token(%s) < ?)", partitionKey)) : "";
    ...
    private static String buildInitialQuery(Read<?> spec, Boolean hasRingRange) {
        return (spec.query() == null) ? String.format("SELECT * FROM %s.%s", spec.keyspace().get(), spec.table().get()) + " WHERE " : spec.query().get() + (hasRingRange ? spec.query().get().toUpperCase().contains("WHERE") ? " AND " : " WHERE " : "");
    }
}