    ...
    List<List<RingRange>> generateSplits(long totalSplitCount, List<BigInteger> ringTokens) {
        ...
        for (int i = 0; i < tokenRangeCount; i++) {
            ...
            for (int j = 0; j < splitCount; j++) {
                 ...
                 * Resolved span issue when both start and end are equals*/
                if (endpointTokens.get(j).compareTo(endpointTokens.get(j + 1)) != 0) {
                    ...
                    LOG.debug("Split are equal and skipping {}: [{},{})", j + 1, endpointTokens.get(j), endpointTokens.get(j + 1));
                }
                /**
