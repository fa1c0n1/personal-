...
public class BaseJavaFunction {
    ...
    public BaseJavaFunction(final NodeService nodeService, final ErrorService errorService){
        this.nodeService = nodeService;
        this.errorService = errorService;
    }

    @SuppressWarnings("unchecked")
    public void initJavaFunctionNode(final StargateNode node,final JavaFunctionOptions options) throws Exception {
        this.nodeName = node.getName();
        this.nodeType = node.getType();
        this.options = options;
        this.options.initSchemaDeriveOptions();
        if (!isBlank(this.options.getSchemaId())) {
            this.schema = PipelineUtils.fetchSchemaWithLocalFallback(options.getSchemaReference(), options.getSchemaId());
            ...
            PipelineUtils.saveLocalSchema(schemaId, schema.toString());
    ...
    private Triple<Function, Class, Boolean> functionInfo() throws Exception {
        ...
        if (this.schemaId == null) {
            ...
            if (!(returnType.isAssignableFrom(GenericRecord.class) || GenericRecord.class.isAssignableFrom(returnType) || returnType.isAssignableFrom(Map.class) || Map.class.isAssignableFrom(returnType))) {
                this.schema = PipelineUtils.fetchSchemaWithLocalFallback(options.getSchemaReference(), returnType.getCanonicalName());
                this.schemaId = schema.getFullName();
                this.converter = converter(this.schema);
        ...
        if (retry == null && options.getRetryOptions() != null) retry = PipelineUtils.retrySettings(nodeName, options.getRetryOptions());
    ...
    private Injector injector() throws Exception {
        ...
        injector = PipelineUtils.getInjector(nodeName, options,nodeService,errorService);
        return injector;
    }
    ...
    public void processElement(final KV<String, GenericRecord> kv, final Consumer<KV<String, GenericRecord>> successConsumer, final Consumer<KV<String, ErrorRecord>> errorConsumer) throws Exception {
        ...
        BaseLogFns.log(options, nodeName, nodeType, kv);
        GenericRecord record = kv.getValue();
        Schema recordSchema = record.getSchema();
        String recordSchemaId = recordSchema.getFullName();
        PipelineUtils.counter(nodeName, nodeType, recordSchemaId, ELEMENTS_IN).inc();
        Object response;
        try {
            ...
            PipelineUtils.histogramDuration(nodeName, nodeType, recordSchemaId, "success_lambda").observe((System.nanoTime() - startTime) / 1000000.0);
        } catch (Exception e) {
            PipelineUtils.histogramDuration(nodeName, nodeType, recordSchemaId, "error_lambda").observe((System.nanoTime() - startTime) / 1000000.0);
            ...
            PipelineUtils.counter(nodeName, nodeType, recordSchemaId, ELEMENTS_ERROR).inc();
            errorConsumer.accept(eRecord(nodeName, nodeType, "invoke_lambda", kv, e));
            return;
        } finally {
            ContextHandler.clearContext();
            PipelineUtils.histogramDuration(nodeName, nodeType, recordSchemaId, "evaluate_lambda").observe((System.nanoTime() - startTime) / 1000000.0);
        }
        if (!emit) {
            PipelineUtils.histogramDuration(nodeName, nodeType, recordSchemaId, "process").observe((System.nanoTime() - startTime) / 1000000.0);
            return;
        }
        if (response == null) {
            PipelineUtils.counter(nodeName, nodeType, recordSchemaId, ELEMENTS_NULL).inc();
        ...
        emitRecords(nodeName, nodeType, kv, record, recordSchema, recordSchemaId, response, this.emit, this.options, schemaMap, converterMap, this.schemaId, this.converter,successConsumer);
    }
    ...
    public static KV<String, GenericRecord> emitOutput(final KV<String, GenericRecord> kv, final String responseKey, final Object response, final Schema schema, final String recordSchemaKey, final String targetSchemaKey, final ObjectToGenericRecordConverter converter, final ConcurrentHashMap<String, ObjectToGenericRecordConverter> converterMap, final String nodeName, final String nodeType,final Consumer<KV<String, GenericRecord>> consumer) throws Exception {
        KV<String, GenericRecord> output = PipelineUtils.consumeOutput(kv, responseKey, response, schema, recordSchemaKey, targetSchemaKey, converter, converterMap, nodeName, nodeType,consumer);
        return output;
    }

    @SuppressWarnings("unchecked")
    public static void emitRecords(final String nodeName, final String nodeType, final KV<String, GenericRecord> kv, final GenericRecord record, final Schema recordSchema, final String recordSchemaId, final Object response, final boolean emit, final DerivedSchemaOptions options, final ConcurrentHashMap<String, Schema> schemaMap, final ConcurrentHashMap<String, ObjectToGenericRecordConverter> converterMap, final String schemaId, final ObjectToGenericRecordConverter converter,final Consumer<KV<String, GenericRecord>> consumer) throws Exception {
        PipelineUtils.consumeOutputRecords(nodeName, nodeType, kv, record, recordSchema, recordSchemaId, response, emit, options, schemaMap, converterMap, schemaId, converter, consumer);
    }
}
