...
public class FileWriter<Input> extends BaseFileWriter<Input> implements Serializable {
    private static final long serialVersionUID = 1L;
    private static final Logger LOGGER = logger(MethodHandles.lookup().lookupClass());

    public FileWriter(final String nodeName, final String nodeType, final PipelineConstants.ENVIRONMENT environment, final FileOptions options) {
    ...
    public void emitMetrics(final String schemaId, final String sourceSchemaId, final String filePath, final SchemaLevelOptions options) {
        incCounters(nodeName, nodeType, schemaId, ELEMENTS_OUT, FILE_PATH, filePath, SOURCE_SCHEMA_ID, sourceSchemaId);
    }
